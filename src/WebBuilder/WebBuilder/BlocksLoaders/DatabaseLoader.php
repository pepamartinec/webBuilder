<?php
namespace WebBuilder\WebBuilder\BlocksLoaders;

use Inspirio\Database\cDBFeederBase;

use WebBuilder\WebBuilder\DataDependencies\UndefinedData;
use WebBuilder\WebBuilder\DataDependencies\InheritedData;
...
use WebBuilder\WebBuilder\BlocksLoaderInterface;
...
class DatabaseLoader implements BlocksLoaderInterface
{
	...
	public function fetchBlocksInstances( BlocksSet $blocksSet )
	{
		...
		foreach( $result as $r ) {
			...
			$block->template         = $r['template_filename'];
	...
	private function fetchInstanceDataInherited( BlockInstance $instance, array &$knownInstances )
	{
		...
		if( $result != null ) {
			foreach( $result as $item ) {
				if( isset( $knownInstances[ $item['source_instance_ID'] ] ) === false ) {
					$knownInstances[ $item['source_instance_ID'] ] = new BlockInstance( $item['source_instance_ID'] );
				}
